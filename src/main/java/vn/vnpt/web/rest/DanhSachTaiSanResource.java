...
import vn.vnpt.service.DanhSachTaiSanQueryService;
...
public class DanhSachTaiSanResource {
    ...
    private final DanhSachTaiSanQueryService danhSachTaiSanQueryService;

    public DanhSachTaiSanResource(
        DanhSachTaiSanService danhSachTaiSanService,
        DanhSachTaiSanRepository danhSachTaiSanRepository,
        DanhSachTaiSanQueryService danhSachTaiSanQueryService
    ) {
        this.danhSachTaiSanService = danhSachTaiSanService;
        this.danhSachTaiSanRepository = danhSachTaiSanRepository;
        this.danhSachTaiSanQueryService = danhSachTaiSanQueryService;
    }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<DanhSachTaiSanDTO>> getAllDanhSachTaiSans(DanhSachTaiSanCriteria criteria) {
        log.debug("REST request to get DanhSachTaiSans by criteria: {}", criteria);

        List<DanhSachTaiSanDTO> entityList = danhSachTaiSanQueryService.findByCriteria(criteria);
        return ResponseEntity.ok().body(entityList);
    }

    /**
     * {@code GET  /danh-sach-tai-sans/count} : count all the danhSachTaiSans.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countDanhSachTaiSans(DanhSachTaiSanCriteria criteria) {
        log.debug("REST request to count DanhSachTaiSans by criteria: {}", criteria);
        return ResponseEntity.ok().body(danhSachTaiSanQueryService.countByCriteria(criteria));
    }
