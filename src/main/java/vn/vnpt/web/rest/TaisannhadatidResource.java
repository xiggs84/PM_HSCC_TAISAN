...
import vn.vnpt.service.TaisannhadatidQueryService;
...
public class TaisannhadatidResource {
    ...
    private final TaisannhadatidQueryService taisannhadatidQueryService;

    public TaisannhadatidResource(
        TaisannhadatidService taisannhadatidService,
        TaisannhadatidRepository taisannhadatidRepository,
        TaisannhadatidQueryService taisannhadatidQueryService
    ) {
        this.taisannhadatidService = taisannhadatidService;
        this.taisannhadatidRepository = taisannhadatidRepository;
        this.taisannhadatidQueryService = taisannhadatidQueryService;
    }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<TaisannhadatidDTO>> getAllTaisannhadatids(TaisannhadatidCriteria criteria) {
        log.debug("REST request to get Taisannhadatids by criteria: {}", criteria);

        List<TaisannhadatidDTO> entityList = taisannhadatidQueryService.findByCriteria(criteria);
        return ResponseEntity.ok().body(entityList);
    }

    /**
     * {@code GET  /taisannhadatids/count} : count all the taisannhadatids.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countTaisannhadatids(TaisannhadatidCriteria criteria) {
        log.debug("REST request to count Taisannhadatids by criteria: {}", criteria);
        return ResponseEntity.ok().body(taisannhadatidQueryService.countByCriteria(criteria));
    }
